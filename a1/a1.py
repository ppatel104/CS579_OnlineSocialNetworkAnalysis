    ...
    while len(q) > 0:
        ...
        for nn in graph.neighbors(n):
            if nn not in seen:
                ...
                if node2distances[nn] < max_depth:
                    q.append(nn)
                    seen.add(nn)
...
def bottom_up(root, node2distances, node2num_paths, node2parents):
    ...
    for n in node2distances:
        node_values[n] = 1.0
    node_dists = sorted(node2distances.items(), key=lambda x: -x[1])
    for n in node_dists:
        if n[0] != root:
            node_parent = node2parents[n[0]]
            if (len(node_parent) == 0):
                break
            elif (len(node_parent) == 1):
                node_values[node_parent[0]] = node_values[node_parent[0]] + node_values[n[0]]
                edge_values[tuple(sorted((node_parent[0], n[0])))] = node_values[n[0]]
            else:
                for p in node_parent:
                    node_values[p] = node_values[p] + (node_values[n[0]] / len(node_parent))
                    edge_values[tuple(sorted((p, n[0])))] = node_values[n[0]] / len(node_parent)
    return edge_values
...
def approximate_betweenness(graph, max_depth):
    ...
    for n in graph.nodes():
        n_dist, n_path, n_parents = bfs(graph, n, max_depth)
        edge_values = bottom_up(n, n_dist, n_path, n_parents)
        e_v = {k: e_v.get(k, 0) + edge_values.get(k, 0) for k in set(e_v) | set(edge_values)}
    e_v = {k: v / 2 for k, v in e_v.items()}
    return e_v
...
def partition_girvan_newman(graph, max_depth):
    ...
    ###TODO
    g_c = graph.copy()
    bet = approximate_betweenness(g_c, max_depth)
    bet = sorted(bet.items(), key=lambda x: (-x[1], x[0]))
    for i in bet:
        g_c.remove_edge(i[0][0], i[0][1])
        sub_comp = list(nx.connected_component_subgraphs(g_c))
        if len(sub_comp) > 1:
            break
    return sub_comp
...
def score_max_depths(graph, max_depths):
    ...
    for md in max_depths:
        sg = partition_girvan_newman(graph, md)
        l.append(tuple((md, norm_cut(sg[0].nodes(), sg[1].nodes(), graph))))
    return l
...
def make_training_graph(graph, test_node, n):
    ...
    """
    ###TODO
    tn_edges = sorted(graph.edges(test_node), key=lambda x: x[1])
    g_c = graph.copy()
    for i in range(0, n):
        g_c.remove_edge(tn_edges[i][0], tn_edges[i][1])
    return g_c
...
def jaccard(graph, node, k):
    ...
    for n in graph.nodes():
        ...
        if not (graph.has_edge(node, n)) and node != n:
            scores.append(tuple(((node, n), len(neighbors & neighbours2) / len(neighbors | neighbours2))))
...
def path_score(graph, root, k, beta):
    ...
    g_c = graph.copy()
    node_dist, node_path, node_parent = bfs(g_c, root, 5)
    node_dist.pop(root)
    for nn in neighbors:
        node_dist.pop(nn)
    for n, v in node_dist.items():
        score.append(tuple(((root, n), (beta ** v) * node_path[n])))
...
def evaluate(predicted_edges, graph):
    ...
    for i in predicted_edges:
        if (graph.has_edge(i[0], i[1])):
            total += 1
    return total / len(predicted_edges)
