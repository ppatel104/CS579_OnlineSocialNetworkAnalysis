...
def vectorize(tokens_list, feature_fns, min_freq, vocab=None):
    ...
    for i in range(len(tokens_list)):
        doc_features[i] = dict((featurize(tokens_list[i],feature_fns)))
        features.append(featurize(tokens_list[i],feature_fns))
    if (vocab==None):
        vocab = {}
        feats = [x[0] for f in features for x in f]
        ...
        for i in sorted(c.keys()):
            if(c[i]>=min_freq):
                vocab[i] = k
                k+=1
    for k,v in doc_features.items():
        for j in v:
            if(j in vocab):
                rows.append(k)
                columns.append(vocab[j])
                data.append(doc_features[k][j])
    matrix = csr_matrix((data,(rows,columns)), shape=(len(tokens_list),len(vocab)))
    return matrix,vocab
    ...
    for punc in punct_vals:
        ...
        for feature in features:
            for freq in min_freqs:
                X,vocab = vectorize(tokens,feature,freq)
                accuracy = cross_validation_accuracy(clf,X,labels,5)
                combination = {}
                combination['punct'] = punc
                combination['features'] = feature
                combination['min_freq'] = freq
                combination['accuracy'] = accuracy
                comb.append(combination)

    return sorted(comb,key = lambda x:(x['accuracy'],x['min_freq']), reverse=True)
...
def parse_test_data(best_result, vocab):
    ...
    be passed to the vectorize function to ensure the fexature
...
if __name__ == '__main__':
    #import doctest
    #doctest.run_docstring_examples(vectorize,globals())
    #doctest.testmod()
    main()