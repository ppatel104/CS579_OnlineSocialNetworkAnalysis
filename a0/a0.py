...
def friend_overlap(users):
    ...
    for i in range(0,len(users)):
        if(i+1>len(users)):
            return sorted(friends_overlap,key=lambda tup:(-tup[2]))
        for j in range(i+1,len(users)):
            friends_overlap = friends_overlap + [(users[i]['screen_name'],users[j]['screen_name'],len(list(set(users[i]['friends']).intersection(users[j]['friends']))))]
            ###print (friends_overlap)
    return sorted(friends_overlap,key=lambda tup:(-tup[2]))
...
def followed_by_hillary_and_donald(users, twitter):
    ...
    """
    ###TODO
    hilary_friends = [x['friends'] for x in users if x['screen_name']=='HillaryClinton']
    trump_friends = [x['friends'] for x in users if x['screen_name']=='realDonaldTrump']
    common_friend = set(hilary_friends[0]).intersection(trump_friends[0])
...
def create_graph(users, friend_counts):
    ...
    for i in users:
        ...
        for friends in i['friends']:
            if(friend_counts[friends]>1):
                graph.add_edge(i['screen_name'],friends)
    return graph
...
def draw_network(graph, users, filename):
    ...
    ###TODO
    figure = plt.figure()
    Graph = nx.draw_networkx(graph,pos=nx.spring_layout(graph),node_size=40,alpha=0.40,width=0.12,font_size=8,labels={user['screen_name']:user['screen_name'] for user in users})
    plt.axis("off")
